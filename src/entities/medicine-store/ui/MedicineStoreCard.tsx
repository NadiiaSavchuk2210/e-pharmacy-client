...
const cardClassName =
  'group relative h-[202px] w-full max-w-[335px] overflow-hidden rounded-[27px] border-[1.15px] border-card-border bg-card-bg shadow-sm content-card-reveal transition-[scale,background-color,border-color,box-shadow] duration-[650ms] ease-[cubic-bezier(0.25,0.1,0.25,1)] hover:scale-[1.02] hover:border-accent hover:bg-accent-soft hover:shadow-md focus-within:scale-[1.02] focus-within:border-accent focus-within:bg-accent-soft focus-within:shadow-md md:h-[232px] md:max-w-none md:w-[calc((100%_-_var(--space-16))_/_2)] lg:w-[calc((100%_-_72px)_/_3)]';
...
const MedicineStoreCard = ({
  ...
  if (!showVisitButton) {
    return (
      <li style={style} className={getCardClassName(showVisitButton)}>
        <Link
          href={href}
          className={cn(
